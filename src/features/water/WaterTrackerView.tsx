...
export function WaterTrackerView() {
  ...
  const todayLog = logs.find(l => l.date === today) || { _id: '', date: today, glasses: 0, goal: 8 }
  ...
  const handleAdd = () => {
    logWater({ date: today, glasses: todayLog.glasses + 1, goal: todayLog.goal }).catch(() => {})
  }
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-text-muted text-xs mt-0.5">Stay hydrated, stay sharp</p>
      </div>
      ...
      <div className="card mb-6 flex flex-col items-center py-8">
        <div className="relative w-48 h-48">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-[10px] text-text-muted">of {todayLog.goal} glasses</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mt-6">
          ...
          <button onClick={handleRemove} className="w-12 h-12 rounded-xl bg-bg-elevated hover:bg-bg-hover flex items-center justify-center transition-colors opacity-0 pointer-events-none">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Avg/Day</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-green-soft">{stats.daysHitGoal}/7</p>
          <p className="text-[9px] text-text-muted">Goal Days</p>
        </div>
        <div className="card text-center">
          ...
          {weekLogs.map(l => {
            ...
            return (
              <div key={l.date} className="flex-1 flex flex-col items-center gap-1">
                <span className="text-[9px] text-text-muted">{l.glasses}</span>
                ...
                <span className={`text-[9px] ${isToday ? 'text-blue-soft font-medium' : 'text-text-muted'}`}>{l.day}</span>
              </div>
            )
          })}
        </div>
