...
export function ReadingListView() {
  ...
  useEffect(() => { fetchBooks().catch(() => {}) }, [fetchBooks])
  ...
  const handleAdd = async () => {
    ...
    await addBook({ ...form, totalPages: parseInt(form.totalPages) || 0, pagesRead: 0, rating: 0, notes: '', startDate: null, finishDate: null }).catch(() => {})
  ...
  const handleProgress = async (book: BookData, pages: number) => {
    ...
    await updateBook(book._id, updates).catch(() => {})
  }
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Reading</p>
        </div>
        <div className="card text-center">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Pages</p>
        </div>
      </div>
                      ...
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="pt-3 mt-3 border-t border-border space-y-2">
                          <div className="flex items-center gap-2">
                            <label className="text-[10px] text-text-muted w-20">Pages read:</label>
                            ...
                            <span className="text-[10px] text-text-muted">/ {book.totalPages}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <label className="text-[10px] text-text-muted w-20">Rating:</label>
                            <div className="flex gap-0.5">
                              {Array.from({ length: 5 }).map((_, i) => (
                                <button key={i} onClick={() => updateBook(book._id, { rating: i + 1 }).catch(() => {})} className="p-0.5">
                                  <Star className={`w-4 h-4 ${i < book.rating ? 'text-accent fill-accent' : 'text-text-muted/30'}`} />
                                </button>
                              ))}
                            </div>
                          </div>
                          <div className="flex items-center gap-2">
                            <label className="text-[10px] text-text-muted w-20">Status:</label>
                            <select className="input text-xs flex-1" value={book.status} onChange={e => updateBook(book._id, { status: e.target.value as BookData['status'] }).catch(() => {})}>
                              <option value="want-to-read">Want to Read</option>
                              <option value="reading">Reading</option>
                              <option value="completed">Completed</option>
                          ...
                          <button onClick={() => deleteBook(book._id).catch(() => {})} className="btn-danger text-[10px] flex items-center gap-1 mt-2"><Trash2 className="w-3 h-3" /> Remove</button>
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
