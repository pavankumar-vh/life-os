...
export function ExpenseTrackerView() {
  ...
  const stats = useMemo(() => {
    const thisMonth = expenses.filter(e => e.date.startsWith(toISODate().slice(0, 7)))
  ...
  const handleSubmit = async () => {
    ...
    await addExpense({ ...form, amount: parseFloat(form.amount) }).catch(() => {})
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-text-muted text-xs mt-0.5">Track your spending</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Transactions</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-red-soft">${stats.recurring.toFixed(0)}</p>
          <p className="text-[9px] text-text-muted">Recurring</p>
        </div>
        <div className="card text-center">
            ...
            {stats.byCategory.map(c => {
              ...
              return (
                <div key={c.id} className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] text-text-muted w-14 text-right">${c.total.toFixed(0)}</span>
                </div>
              )
            })}
          </div>
              ...
              return (
                <motion.div key={e._id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="card flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-text-muted">{formatDate(e.date)} {e.recurring && 'Recurring'}</p>
                  </div>
                  <span className="text-sm font-semibold text-accent">${e.amount.toFixed(2)}</span>
                  <button onClick={() => deleteExpense(e._id).catch(() => {})} className="btn-ghost p-1"><Trash2 className="w-3 h-3" /></button>
                </motion.div>
              )
            })}
          </AnimatePresence>
