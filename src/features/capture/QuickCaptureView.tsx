...
export function QuickCaptureView() {
  ...
  useEffect(() => { fetchCaptures().catch(() => {}) }, [fetchCaptures])
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Inbox</p>
        </div>
        <div className="card text-center">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Thoughts</p>
        </div>
      </div>
            ...
            return (
              <motion.div key={item._id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                ...
                <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all shrink-0">
                  <button onClick={() => updateCapture(item._id, { processed: !item.processed })}
                    className={`p-1 rounded ${item.processed ? 'text-text-muted' : 'text-green-soft hover:bg-green-soft/10'}`} title="Toggle processed">
                    <Check className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => deleteCapture(item._id)} className="text-text-muted hover:text-red-soft p-1">
