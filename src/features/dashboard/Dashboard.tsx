  ...
  return (
    <div className="max-w-4xl mx-auto">
          ...
          <motion.div variants={stagger.item} className="card">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <span className="badge text-[10px]">{user.xp} XP</span>
              </div>
      ...
      >
        {stats.map((stat) => (
          ...
          >
            ...
            <p className="text-[10px] text-text-muted mt-1.5">{stat.sub}</p>
            <p className="text-[10px] text-accent mt-2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all duration-300">
          ...
          {weeklyHabitData.map(({ date, completed, total }, idx) => {
            ...
            return (
              <div key={date} className="flex-1 flex flex-col items-center gap-1.5">
                ...
                <span className={`text-[10px] ${isToday ? 'text-accent font-bold' : 'text-text-muted'}`}>{dayNames[dayIdx]}</span>
              </div>
            )
          })}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
        ...
        >
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-text-muted">{totalHabits - completedHabitsToday} remaining</p>
            </div>
          </div>
          {habits.length === 0 ? (
                ...
                return (
                  ...
                  >
                    <motion.div
                      ...
                      style={{ backgroundColor: done ? '#4ade80' : 'rgba(255,255,255,0.06)' }}
                      animate={done ? { boxShadow: '0 0 8px rgba(74, 222, 128, 0.4)' } : {}}
                    />
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-medium text-text-muted flex items-center gap-1.5"><Utensils className="w-3.5 h-3.5 text-accent" /> Nutrition</span>
          ...
          <div className="space-y-4">
            {[
              { label: 'Calories', value: todayCalories, max: 2200, unit: 'kcal', color: '#e8d5b7', glow: 'rgba(232, 213, 183, 0.3)' },
              { label: 'Protein', value: todayProtein, max: 150, unit: 'g', color: '#34d399', glow: 'rgba(52, 211, 153, 0.3)' },
              { label: 'Carbs', value: todayCarbs, max: 250, unit: 'g', color: '#60a5fa', glow: 'rgba(96, 165, 250, 0.3)' },
              { label: 'Fat', value: todayFat, max: 70, unit: 'g', color: '#fb923c', glow: 'rgba(251, 146, 60, 0.3)' },
            ].map((m, idx) => (
              <div key={m.label}>
          ...
          <p className="text-[10px] text-text-muted mt-4 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-medium text-text-muted flex items-center gap-1.5"><Dumbbell className="w-3.5 h-3.5 text-blue-soft" /> Gym</span>
          ...
          {todayWorkouts.length === 0 ? (
            <div className="text-center py-8">
              ...
              >
                <Dumbbell className="w-10 h-10 text-text-muted/20 mx-auto mb-3" />
              </motion.div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] text-text-muted">{w.duration}min</span>
                    </div>
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-medium text-text-muted flex items-center gap-1.5"><CheckSquare className="w-3.5 h-3.5 text-green-soft" /> Tasks & Goals</span>
          ...
          {urgentTasks.length > 0 && (
            <div className="mb-4">
              <p className="text-[10px] text-red-soft font-semibold mb-2 uppercase tracking-wider flex items-center gap-1">
              ...
              {urgentTasks.slice(0, 3).map((t) => (
                ...
                >
                  ...
                  <span className="badge text-[10px]">{t.status}</span>
                </motion.div>
              ))}
            </div>
          )}
          {closestGoal && (
            <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <p className="text-[10px] text-text-muted mt-1.5">{closestGoal.progress}/{closestGoal.target} {closestGoal.unit}</p>
            </div>
          )}
          <div className="flex items-center justify-between mt-4 pt-3" style={{ borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <span className="text-[10px] text-text-muted">{activeGoals} active goals</span>
          </div>
        </motion.div>
      </div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <span className="text-xs font-medium text-text-muted flex items-center gap-1.5"><Brain className="w-3.5 h-3.5 text-purple-soft" /> Journal</span>
        ...
        {todayEntry ? (
          ...
          >
            ...
            {todayEntry.tags?.length > 0 && (
              <div className="flex gap-1 mt-2">{todayEntry.tags.map((t: string) => <span key={t} className="badge text-[10px]">{t}</span>)}</div>
            )}
          </motion.div>
        ) : (
          <div className="text-center py-8">
      ...
      >
        {[
          ...
          <motion.div
            ...
            whileHover={{ y: -2, boxShadow: `0 8px 32px -8px ${s.glow}` }}
          >
            <div
