  ...
  return (
    <div>
      ...
      <div className="card mb-6 border-accent/10 bg-gradient-to-br from-bg-surface to-bg-elevated/50">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-text-muted mt-1">— {quote.author}</p>
          </div>
        </div>
      </div>
      ...
      <div className="card mb-6 text-center">
        ...
        <p className="text-[9px] text-text-muted mt-2">Based on habits, tasks, mood, sleep & workouts</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Habit Rate</p>
        </div>
        <div className="card text-center">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Tasks Done</p>
        </div>
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Avg Sleep</p>
        </div>
      </div>
        ...
        <div className="space-y-2.5">
          {reviewData.habitCompletions.map(h => (
            <div key={h.name} className="flex items-center gap-3">
              ...
              <span className="text-[10px] text-text-muted w-14 text-right">{h.completed}/{h.total}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div className="card">
          ...
          <div className="flex items-center justify-between mb-2">
            <span className="text-[10px] text-text-muted">{reviewData.weekJournals.length} entries this week</span>
            <span className="text-[10px] text-text-muted flex items-center gap-1">Avg mood: <MoodIcon mood={Math.round(reviewData.avgMood) || 3} size={12} /> {reviewData.avgMood}/5</span>
          </div>
            ...
            <div className="flex gap-1">
              {reviewData.weekJournals.map(j => (
                <div key={j._id} className="flex-1 text-center p-2 bg-bg-elevated/50 rounded-lg">
                  ...
                  <p className="text-[8px] text-text-muted mt-0.5">{j.date.slice(5)}</p>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-3 gap-2 text-center">
            <div className="p-2 bg-bg-elevated/50 rounded-lg">
              ...
              <p className="text-[8px] text-text-muted">Workouts</p>
            </div>
            <div className="p-2 bg-bg-elevated/50 rounded-lg">
              <p className="text-lg font-bold text-accent">{Math.round(reviewData.totalVolume / 1000)}k</p>
              <p className="text-[8px] text-text-muted">Volume (kg)</p>
            </div>
            <div className="p-2 bg-bg-elevated/50 rounded-lg">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-2 gap-2 text-center">
            <div className="p-2 bg-bg-elevated/50 rounded-lg">
              ...
              <p className="text-[8px] text-text-muted">Avg Hours</p>
            </div>
            <div className="p-2 bg-bg-elevated/50 rounded-lg">
              <p className="text-lg font-bold text-green-soft">{reviewData.avgSleepQuality}/5</p>
              <p className="text-[8px] text-text-muted">Avg Quality</p>
            </div>
          </div>
        </div>
          ...
          {reviewData.latestBody ? (
            <div className="grid grid-cols-2 gap-2 text-center">
              <div className="p-2 bg-bg-elevated/50 rounded-lg">
                ...
                <p className="text-[8px] text-text-muted">Weight (kg)</p>
              </div>
              <div className="p-2 bg-bg-elevated/50 rounded-lg">
            ...
            <p className="text-[10px] text-text-muted text-center py-4">No body data yet</p>
          )}
        </div>
      </div>
    </div>
