...
export function FlashcardsView() {
  ...
  useEffect(() => { fetchCards().catch(() => {}) }, [fetchCards])
  ...
  const handleAdd = async () => {
    ...
    await addCard({ ...form, nextReview: toISODate(), timesReviewed: 0, timesCorrect: 0 }).catch(() => {})
    ...
    <div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        ...
        <div className="card text-center">
          ...
          <p className="text-[9px] text-text-muted">Due</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-green-soft">{stats.mastered}</p>
          <p className="text-[9px] text-text-muted">Mastered</p>
        </div>
        <div className="card text-center">
          <p className="text-2xl font-bold text-blue-soft">{stats.decks}</p>
          <p className="text-[9px] text-text-muted">Decks</p>
        </div>
      </div>
      ...
      {mode === 'study' && currentCard && (
        <div className="mb-6">
          <div className="card min-h-[200px] flex flex-col items-center justify-center text-center cursor-pointer" onClick={() => setShowAnswer(!showAnswer)}>
            <span className={`text-[9px] px-2 py-0.5 rounded-full mb-3 ${currentCard.difficulty === 'easy' ? 'bg-green-soft/15 text-green-soft' : currentCard.difficulty === 'hard' ? 'bg-red-soft/15 text-red-soft' : 'bg-accent/15 text-accent'}`}>{currentCard.difficulty} · {currentCard.deck}</span>
            <AnimatePresence mode="wait">
              {!showAnswer ? (
                <motion.div key="front" initial={{ rotateY: -90 }} animate={{ rotateY: 0 }} exit={{ rotateY: 90 }} transition={{ duration: 0.2 }}>
                  ...
                  <p className="text-[10px] text-text-muted mt-3">Click to reveal answer</p>
                </motion.div>
              ) : (
          ...
          <p className="text-center text-[10px] text-text-muted mt-3">{studyIdx + 1} / {studyCards.length}</p>
        </div>
      )}
      ...
      {mode === 'study' && !currentCard && (
        <div className="card text-center py-12 mb-6">
          ...
          <p className="text-[10px] text-text-muted mt-1">No cards due right now. Add more or come back later.</p>
        </div>
      )}
              ...
              return (
                <motion.div key={card._id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="card flex items-center gap-3 group">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] text-text-muted">{card.deck} · {accuracy}% accuracy · {card.timesReviewed}x reviewed</p>
                  </div>
                  <button onClick={() => deleteCard(card._id).catch(() => {})} className="btn-ghost p-1 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 className="w-3 h-3" /></button>
                </motion.div>
              )
            })}
          </AnimatePresence>
