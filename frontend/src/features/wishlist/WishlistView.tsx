            ...
            {filtered.map(item => {
              ...
              return (
                <motion.div key={item._id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className={`card flex items-center gap-3 group ${item.completed ? 'opacity-60' : ''}`}>
                  ...
                  <button onClick={() => { if (confirm('Delete this item?')) deleteItem(item._id).catch(() => toast.error('Failed to delete')) }} className="btn-ghost p-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity"><Trash2 className="w-3 h-3" /></button>
                </motion.div>
              )
            })}
          </AnimatePresence>
