...
export function TasksView() {
  ...
  useEffect(() => { fetchTasks().catch(() => toast.error('Failed to load tasks')); fetchGoals().catch(() => toast.error('Failed to load goals')) }, [fetchTasks, fetchGoals])
  ...
  const handleAdd = async () => {
    ...
    await addTask({ title, description, priority, status: 'todo', dueDate: dueDate || null, goalId: goalId || null }).catch(() => toast.error('Failed to add task'))
  ...
  const cycleStatus = async (task: TaskData) => {
    ...
    await updateTask(task._id, { status: next[task.status] }).catch(() => toast.error('Failed to update task'))
  }
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden mb-6">
            <div className="card border-accent/30">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
                <div>
                  <label className="text-xs text-text-secondary uppercase block mb-1">Priority</label>
              ...
              <motion.div key={task._id} layout initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-[11px] px-1.5 py-0.5 rounded-full font-medium ${pri.color} ${pri.bg}`}>{pri.label}</span>
                  </div>
                ...
                <div className="flex items-center gap-1 md:opacity-0 md:group-hover:opacity-100 transition-all shrink-0">
                  <button onClick={() => { if (confirm('Delete this task?')) deleteTask(task._id).catch(() => toast.error('Failed to delete')) }} className="text-text-muted hover:text-red-soft"><Trash2 className="w-3.5 h-3.5" /></button>
                </div>
              </motion.div>
            )
          })}
