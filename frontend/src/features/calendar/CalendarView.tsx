    ...
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
        <div className="card text-center">
          ...
          <p className="text-xs text-text-secondary">Events this month</p>
        </div>
        <div className="card text-center">
          ...
          <p className="text-xs text-text-secondary">Active days</p>
        </div>
        <div className="card text-center">
        ...
        {TYPE_FILTERS.map(t => {
          ...
          return (
            <button key={t} onClick={() => setTypeFilter(t)}
              ...
              {t !== 'all' && monthStats.types[t] ? <span className="text-[11px] opacity-70">({monthStats.types[t]})</span> : null}
            </button>
          )
        })}
      </div>
          ...
          <div className="card">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-1">
                <button onClick={goToToday} className="text-xs text-accent hover:underline mr-2">Today</button>
                <button onClick={() => navigateMonth(1)} className="p-1.5 hover:bg-bg-hover rounded-lg transition-colors">
            ...
            <div className="mt-4 pt-4 border-t border-border">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-text-muted font-medium">Activity Distribution</span>
              </div>
                ...
                {Object.entries(monthStats.types).map(([type, count]) => {
                  ...
                  return (
                    <div key={type} className={`flex items-center gap-1.5 px-2 py-1 rounded-md ${cfg.bg}`}>
                      ...
                      <span className={`text-xs font-medium ${cfg.color}`}>{count} {cfg.label.toLowerCase()}</span>
                    </div>
                  )
                })}
              </div>
                    ...
                    return (
                      <motion.div key={i} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {event.subtitle && <p className="text-xs text-text-muted mt-0.5">{event.subtitle}</p>}
              ...
              .map(([date, evts]) => (
                <div key={date} className="mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-[11px] text-text-secondary">{evts.length} events</span>
                  </div>
                    ...
                    {evts.map((event, i) => {
                      ...
                      return (
                        <motion.div key={i} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }}
                          ...
                          <span className={`text-[11px] px-1.5 py-0.5 rounded ${cfg?.bg || ''} ${cfg?.color || ''}`}>{cfg?.label}</span>
                        </motion.div>
                      )
                    })}
                  </div>
