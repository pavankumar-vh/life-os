...
import { Zap, ArrowRight, Sparkles, Mail, CheckCircle } from 'lucide-react'
...
export function AuthScreen() {
  ...
  const reset = () => { setError(''); setForgotSent(false); setEmail(''); setPassword(''); setName('') }
    ...
    <div className="min-h-screen bg-bg flex items-center justify-center p-4 overflow-hidden relative">
      ...
      >
        ...
        <motion.div className="text-center mb-10"
          ...
          <motion.div className="inline-flex items-center justify-center w-16 h-16 rounded-2xl mb-5 relative"
            style={{ background: 'rgba(232,213,183,0.1)', border: '1px solid rgba(232,213,183,0.15)', boxShadow: '0 8px 32px -4px rgba(232,213,183,0.2), inset 0 1px 0 rgba(255,255,255,0.1)', backdropFilter: 'blur(16px)' }}>
            <Zap className="w-6 h-6 text-accent" />
          </motion.div>
          <motion.h1 className="text-4xl font-bold tracking-tight gradient-text"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4, duration: 0.6 }}>
            LifeOS
          </motion.h1>
          <motion.p className="text-text-muted text-sm mt-2 flex items-center justify-center gap-1.5"
        ...
        {/* Card */}
        <motion.div className="rounded-2xl p-7 glow-card"
          style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(20px) saturate(150%)', WebkitBackdropFilter: 'blur(20px) saturate(150%)', boxShadow: '0 16px 48px -12px rgba(0,0,0,0.6), inset 0 1px 0 rgba(255,255,255,0.08)' }}
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
            ...
            {mode === 'forgot' && (
              <motion.div key="forgot" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }}>
                <button onClick={() => { setMode('login'); reset() }}
                  className="flex items-center gap-1 text-[11px] text-text-muted hover:text-text-secondary mb-5 transition-colors cursor-pointer">
                  ← Back to login
                </button>
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    <h2 className="text-sm font-semibold text-text-primary">Forgot your password?</h2>
                    <p className="text-[10px] text-text-muted">We'll send you a reset link</p>
                  </div>
                </div>
                ...
                {forgotSent ? (
                  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                    ...
                    <p className="text-xs text-text-muted">A reset link has been sent to <strong className="text-text-secondary">{email}</strong>. Expires in 30 minutes.</p>
                  </motion.div>
                ) : (
                  <form onSubmit={handleForgot} className="space-y-3">
                    <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                      placeholder="Enter your email" required className="input w-full" />
                    <AnimatePresence>
                      {error && (
                    ...
                    <motion.button type="submit" disabled={forgotLoading}
                      className="btn w-full py-3 text-sm group"
                      whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}>
                      {forgotLoading ? (
                        <motion.div className="w-4 h-4 border-2 border-bg/30 border-t-bg rounded-full"
                          animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: 'linear' }} />
                      ) : <>Send Reset Link <ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" /></>}
                    </motion.button>
                  </form>
                )}
              </motion.div>
              ...
              <motion.div key="auth" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                ...
                <div className="flex mb-7 relative">
                  ...
                  {(['login', 'register'] as const).map((tab) => (
                    <button key={tab} onClick={() => { setMode(tab); reset() }}
                      className={`flex-1 pb-3 text-sm font-medium transition-all duration-300 relative capitalize ${mode === tab ? 'text-accent' : 'text-text-muted hover:text-text-secondary'}`}>
                      {tab === 'login' ? 'Login' : 'Register'}
                      {mode === tab && (
                        <motion.div layoutId="auth-tab"
                          className="absolute bottom-0 left-[20%] right-[20%] h-[2px] rounded-full"
                          style={{ background: 'linear-gradient(90deg, transparent, #e8d5b7, transparent)', boxShadow: '0 0 12px rgba(232,213,183,0.3)' }}
                ...
                <motion.button onClick={handleGoogleSignIn}
                  ...
                  whileHover={{ background: 'rgba(255,255,255,0.09)', scale: 1.01 }} whileTap={{ scale: 0.98 }}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {mode === 'register' && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                        ...
                        <input type="text" value={name} onChange={e => setName(e.target.value)}
                          placeholder="Name" required={mode === 'register'} className="input w-full" />
                      </motion.div>
                    )}
                  </AnimatePresence>

                  <input type="email" value={email} onChange={e => setEmail(e.target.value)}
                    placeholder="Email" required className="input w-full" />
                  <div>
                    <input type="password" value={password} onChange={e => setPassword(e.target.value)}
                      placeholder="Password" required minLength={6} className="input w-full" />
                    {mode === 'login' && (
                  ...
                  <AnimatePresence>
                    {error && (
                      <motion.div initial={{ opacity: 0, y: -5, height: 0 }} animate={{ opacity: 1, y: 0, height: 'auto' }}
                  ...
                  <motion.button type="submit" disabled={isLoading}
                    ...
                    whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}>
                    {isLoading ? (
                      <motion.div className="w-5 h-5 border-2 border-bg/30 border-t-bg rounded-full"
                        animate={{ rotate: 360 }} transition={{ duration: 0.8, repeat: Infinity, ease: 'linear' }} />
                    ) : (
                      <>
        ...
        <motion.p className="text-center text-text-muted text-xs mt-6 tracking-wide"
