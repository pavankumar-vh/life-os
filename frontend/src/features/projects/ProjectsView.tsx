                ...
                <motion.div key={project._id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="card">
                  ...
                  <div className="flex items-center gap-3 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : project._id)}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {project.description && <p className="text-xs text-text-muted mt-0.5 truncate">{project.description}</p>}
                    </div>
                    ...
                    {isExpanded && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                        <div className="pt-3 mt-3 border-t border-border">
                          {project.deadline && <p className="text-xs text-text-muted mb-2">Deadline: {formatDate(project.deadline)}</p>}
                          ...
                          <div className="flex items-center gap-2 mt-3 pt-2 border-t border-border">
                            <select className="input text-xs flex-1" value={project.status} onChange={e => updateProject(project._id, { status: e.target.value as ProjectData['status'] }).catch(() => toast.error('Failed to update status'))}>
                              <option value="active">Active</option>
                              <option value="completed">Completed</option>
                              <option value="on-hold">On Hold</option>
                              <option value="archived">Archived</option>
                            </select>
                            <button onClick={() => { if (confirm('Delete this project?')) deleteProject(project._id).catch(() => toast.error('Failed to delete')) }} className="btn-danger text-xs flex items-center gap-1"><Trash2 className="w-3 h-3" /> Delete</button>
                          </div>
                        </div>
                      </motion.div>
                    )}
