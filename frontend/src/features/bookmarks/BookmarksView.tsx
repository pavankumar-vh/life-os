                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                      <span className="text-[11px] bg-bg-elevated px-1.5 py-0.5 rounded text-text-muted">{bm.folder}</span>
                      {bm.tags.map(t => (
                        <span key={t} className="text-[11px] bg-accent/10 text-accent px-1.5 py-0.5 rounded">#{t}</span>
                      ))}
                    </div>
                  </div>
                  <button onClick={() => { if (confirm('Delete this bookmark?')) deleteBookmark(bm._id).catch(() => toast.error('Failed to delete')) }} className="btn-ghost p-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity"><Trash2 className="w-3 h-3" /></button>
                </div>
              </motion.div>
            ))}
          </AnimatePresence>
