...
router.post('/', async (req: AuthRequest, res) => {
  try {
    ...
    audit(userId, 'create', 'wishlist', item._id, { after: item.toJSON() })
...
router.put('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    audit(userId, 'update', 'wishlist', id, { after: item.toJSON(), changes: updates as Record<string, unknown> })
...
router.delete('/:id', async (req: AuthRequest, res) => {
  try {
    ...
    const item = await WishlistItem.findOne({ _id: id, userId })
    if (item) audit(userId, 'delete', 'wishlist', id, { before: item.toJSON() })
